...
function encrypt(string,encryptionExponent, modulus, maxdigits) {
	var keyPair = jCryptionKeyPair(encryptionExponent, modulus, maxdigits);
	var charSum = 0;
...
function encryption(encryptObject, keyPair) {
	return encryptChar(encryptObject, keyPair);
}
	
function encryptChar(encryptObject, keyPair) {
	var charCounter = 0;
	var j, block;
	var encrypted = "";
	block = new BigInt();
...
function jCryptionKeyPair(encryptionExponent, modulus, maxdigits) {
	...
	object.e = biFromHex(encryptionExponent);
	object.m = biFromHex(modulus);
	object.chunkSize = 2 * biHighIndex(object.m);
	object.radix = 16;
	object.barrett = new BarrettMu(object.m);
	return object;
}
