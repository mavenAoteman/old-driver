...
function test(p, m , e, n){
	setMaxDigits(m);
	return (encryptedString(new RSAKeyPair(e, "", n), p));
}
...
function biFromDecimal(e) {
	...
	if (c == e.length) {
		...
		c += f;
		while (c < e.length) {
			a = biAdd(biMultiply(a, lr10), biFromNumber(Number(e.substr(c,
					dpl10))));
			c += dpl10
		}
		a.isNeg = d
...
function biShiftLeft(b, h) {
	...
	for ( var e = a.digits.length - 1, f = e - 1; e > 0; --e, --f) {
		a.digits[e] = ((a.digits[e] << g) & maxDigitVal)
				| ((a.digits[f] & highBitMasks[g]) >>> (c))
	}
...
function biShiftRight(b, h) {
	...
	for ( var d = 0, e = d + 1; d < a.digits.length - 1; ++d, ++e) {
		a.digits[d] = (a.digits[d] >>> f)
				| ((a.digits[e] & lowBitMasks[f]) << g)
	}
